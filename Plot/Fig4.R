...
Filter<-data.frame()
for (i in 1:7) {
  Mutation<-read.table(paste0("/Dell/Dell13/shankj/projects/Cov/TranscriptionError/Cirseq_Human/SNP/bulkRNA/",temp[i]),sep="\t",header = F,stringsAsFactors = F)
  colnames(Mutation)<-c("Strand","Chr","Site","Total_reads","SNP","Number","Fraction","Reads","Ref_Reads")
  Mutation$SNP_Ref<-Mutation$Ref_Reads/(Mutation$Number/Mutation$Fraction)
  Mid<-as.data.frame(filter(Mutation, !SNP %in% c('SNP'),!Chr=="MT", Fraction<=0.05, SNP_Ref>=0.95) %>% #Number==1,Chr %in% c('Poliovirus,'),
  ...
  plotmid<-as.data.frame(Mid  %>% group_by(Chr,Site,SNP)  %>% dplyr::summarise(count=n()))
  ...
  plot_list[[temp[i]]]<-ggplot(data=plot, aes(x=factor(SNP),y=Count,fill=SNP)) + 
    geom_bar(position="dodge", stat="identity",color="black",width = 0.8)+
    scale_fill_manual(values = Self)+
    labs(x='',y='Transcript error',title = name)+my_theme
...
#tail(Filter)
#nrow(filter(Filter,grepl(Chr,"MT")))

plotmid<-as.data.frame(Filter  %>% group_by(Chr,Site,SNP)  %>% dplyr::summarise(count=n()))
plot<-as.data.frame(filter(plotmid,count==1) %>% group_by(SNP)  %>% dplyr::summarise(Count=sum(count)))

#plotmid<-as.data.frame(Filter  %>% group_by(Chr,Site,SNP,sample)  %>% dplyr::summarise(count=n()))
#plot<-as.data.frame(filter(plotmid) %>% group_by(SNP,sample)  %>% dplyr::summarise(Count=n()))
#plot<-as.data.frame(Filter  %>% group_by(Chr,Site,SNP,sample)  %>% dplyr::summarise(Count=sum(Number)))
plot$SNP<-factor(plot$SNP,levels = c("C > T","G > A","A > G","T > C","G > T","C > A","G > C","C > G","A > C","T > G","A > T","T > A"))


pdf("/Dell/Dell13/shankj/projects/Cov/Plot/Fig4/Cirseq_Hela.pdf",width = 2,height = 2)
ggplot(data=plot, aes(x=factor(SNP),y=Count/10^3,fill=SNP)) + 
  geom_bar(position="dodge", stat="identity",color="black",width = 0.8)+
  scale_fill_manual(values = Self)+
  labs(x='',y='Transcript error(x10^3)')+my_theme
dev.off()

#####################################################################

#做检验
temp <- list.files(path = "/Dell/Dell13/shankj/projects/Cov/TranscriptionError/Cirseq_Human/",pattern = "*.content100")
head(Mutation)
Filter<-data.frame()
for (i in 1:7) {
  Mutation<-read.table(paste0("/Dell/Dell13/shankj/projects/Cov/TranscriptionError/Cirseq_Human/",temp[i]),sep="\t",header = T,stringsAsFactors = F)
  ...
  name=sub(".content100","",temp[i])
  merge$sample<-rep(name,nrow(merge))
  Filter<-rbind(Filter,merge)
}

head(Base_pos_count)
head(Filter)
#以基因组中>=100 reads覆盖的位点作为background，如果在p2-p8中有同一位点，只计算一次
Base_pos_count<-Filter %>% group_by(Chr,Site,count,Base,Count)  %>% dplyr::summarise(Pos_number=n())  %>% #相同的位点只计算一遍
...
# G     84850
# T     76434
#fisher.test(matrix(c(651,	483,(84850-862),(81145-730)),nrow=2))
#fisher.test(matrix(c(805,	631,(84850-805),(81145-631)),nrow=2))

#以p2-p8中>=100 reads覆盖的位点的所有reads作为background
Base_read_count<-Filter %>% group_by(Base) %>% dplyr::summarise(Count=sum(count))
#Base_read_count<-Filter %>% group_by(Base,sample) %>% dplyr::summarise(Count=sum(count))
...
#fisher.test(matrix(c(805,	631,(85238433-805),(76631950-631)),nrow=2))
#fisher.test(matrix(c(1177,	1092,(85238433-1177),(76631950-1092)),nrow=2))

P1<-plot
P2<-plot
Lynch_Me<-merge(P1,P2,by="SNP")
cor.test(Lynch_Me$Count.x,Lynch_Me$Count.y,method = "s")
#############################################################################################################################################################################################################################
