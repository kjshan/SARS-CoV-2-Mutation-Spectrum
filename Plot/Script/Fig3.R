...
#########################################################################
Count<-read.table("/Dell/Dell13/shankj/projects/Cov/Genome/NegativeStrand/Kejia/ssDNA/HepatitisB/HepatitisB_Astr_Align.SNP",sep="\t",header = F,stringsAsFactors = F)
...
#pdf("Mumps_genotypeG_polymorphism.pdf",width = 2,height = 2)
ggplot(data=plot, aes(x=factor(SNP),y=count,fill=SNP)) + 
  geom_bar(position="dodge", stat="identity",color="black",width = 0.8)+
  scale_fill_manual(values = Self)+
  labs(x='',y='SNV count')+my_theme
...
fisher.test(matrix(c(27,	29,(3373-27),(3093-29)),nrow=2))
...
#########################################################################
Count<-read.table("/Dell/Dell13/shankj/projects/Cov/Genome/NegativeStrand/Kejia/PV123/V01149/V01149_100_Astr_Align.SNP",sep="\t",header = F,stringsAsFactors = F)
