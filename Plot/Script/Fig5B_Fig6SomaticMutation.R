...
mid<-somatic %>% group_by(V2,V3,V5)   %>% dplyr::summarise(count=n()) 
...
ggplot(data=filter(count,V6=="Whole_Blood"), aes(x=V5,y=count,fill=V5)) + #geom_abline(intercept=0.5, slope=0,color="red")+
  geom_bar(position="dodge", stat="identity",width = 0.8)+
  scale_fill_manual(values = Self)+#scale_y_continuous(breaks = seq(0,2000,500),limits = c(0,2000))+
  labs(x='SNP',y='count',title = i)+#+#facet_grid(~`Sample.Type`)
  my_theme

plot_list=list()
for (i in unique(count$V6)) {
  plot_list[[i]]<-ggplot(data=filter(count,V6==i), aes(x=V5,y=count,fill=V5)) + #geom_abline(intercept=0.5, slope=0,color="red")+
    geom_bar(position="dodge", stat="identity",width = 0.8)+
    scale_fill_manual(values = Self)+#scale_y_continuous(breaks = seq(0,2000,500),limits = c(0,2000))+
    labs(x='SNP',y='count',title = i)+#+#facet_grid(~`Sample.Type`)
    my_theme#+facet_grid(~hl)#2+guides(fill=F)
...
ggplot() + 
  ...
  ggrepel::geom_text_repel(data =odds, aes(x=log2(odds2),y=log2(odds1),label = seq),col="black",segment.size = 1,min.segment.length = 1)+
